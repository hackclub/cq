...
export function getConfig(overrides = {}) {
  ...
  return {
    ...
    r2Endpoint: overrides.r2Endpoint ?? process.env.R2_ENDPOINT ?? "",
    r2AccessKeyId: overrides.r2AccessKeyId ?? process.env.R2_ACCESS_KEY_ID ?? "",
    r2SecretAccessKey: overrides.r2SecretAccessKey ?? process.env.R2_SECRET_ACCESS_KEY ?? "",
    r2BucketName: overrides.r2BucketName ?? process.env.R2_BUCKET_NAME ?? "hackclub-cq",
    r2PublicBaseUrl: (overrides.r2PublicBaseUrl ?? process.env.R2_PUBLIC_BASE_URL ?? "https://cq.hackclub-assets.com").replace(/\/+$/, ""),
    reviewEnvironmentMaxActive: integer(overrides.reviewEnvironmentMaxActive ?? process.env.REVIEW_ENVIRONMENT_MAX_ACTIVE, 4),
    reviewEnvironmentTtlMinutes: integer(overrides.reviewEnvironmentTtlMinutes ?? process.env.REVIEW_ENVIRONMENT_TTL_MINUTES, 120),
    reviewAgentUrl: (overrides.reviewAgentUrl ?? process.env.REVIEW_AGENT_URL ?? "").replace(/\/+$/, ""),
    reviewAgentToken: overrides.reviewAgentToken ?? process.env.REVIEW_AGENT_TOKEN ?? "",
    cloudflareAccessClientId: overrides.cloudflareAccessClientId ?? process.env.CF_ACCESS_CLIENT_ID ?? "",
    cloudflareAccessClientSecret: overrides.cloudflareAccessClientSecret ?? process.env.CF_ACCESS_CLIENT_SECRET ?? "",
    proxmoxApiUrl: overrides.proxmoxApiUrl ?? process.env.PROXMOX_API_URL ?? "",
    proxmoxApiToken: overrides.proxmoxApiToken ?? process.env.PROXMOX_API_TOKEN ?? "",
    proxmoxNode: overrides.proxmoxNode ?? process.env.PROXMOX_NODE ?? "pve",
    proxmoxStorage: overrides.proxmoxStorage ?? process.env.PROXMOX_STORAGE ?? "local-lvm",
    proxmoxTemplateVmid: integer(overrides.proxmoxTemplateVmid ?? process.env.PROXMOX_TEMPLATE_VMID, 9000),
    proxmoxVmidStart: integer(overrides.proxmoxVmidStart ?? process.env.PROXMOX_VMID_START, 9100),
