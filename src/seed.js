...
export async function seedStore(store) {
  ...
  if (!process.env.NODE_TEST_CONTEXT && !(await store.get("project", "cq_reviewer_training"))) {
    ...
    await store.put("journal", "log_reviewer_training_1", { id: "log_reviewer_training_1", projectId: "cq_reviewer_training", title: "Built the signal parser", text: "Implemented the parser and documented the sample signal data.", imageUrl: "https://example.com/training-parser.png", imageUrls: ["https://example.com/training-parser.png"], entryDate: timestamp.slice(0, 10), minutes: 35, hackatimeSeconds: 2100, createdAt: timestamp, updatedAt: timestamp });
    await store.put("journal", "log_reviewer_training_2", { id: "log_reviewer_training_2", projectId: "cq_reviewer_training", title: "Tested the display", text: "Tested the visualizer with recorded amateur-radio measurements and fixed an edge case.", imageUrl: "https://example.com/training-display.png", imageUrls: ["https://example.com/training-display.png"], entryDate: timestamp.slice(0, 10), minutes: 25, hackatimeSeconds: 1500, createdAt: timestamp, updatedAt: timestamp });
  }
}
