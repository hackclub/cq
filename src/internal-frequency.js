...
function issue(config, user, session, page) {
  ...
  HEADER.copy(header); header.writeUInt8(2, 4); header.writeUInt32BE(Math.floor(Date.now() / 1000), 5); header.writeUInt8(pageCode(page), 9);
  ...
  for (let tileRow = 0; tileRow < ROWS / TILE_ROWS; tileRow += 1) for (let tileColumn = 0; tileColumn < COLUMNS / TILE_COLUMNS; tileColumn += 1) {
    stream.forEach((value, index) => {
      for (let copy = 0; copy < 3; copy += 1) {
        const position = positions[index * 3 + copy]; const localCell = Math.floor(position / CHANNELS);
