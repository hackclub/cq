...
export function authRoutes({ store, config }) {
  ...
  router.post("/dev-login", rateLimit, async (req, res) => {
    ...
    await store.put("user", id, {
      ...
      lastName: existing?.lastName ?? "Radio",
      birthday: existing?.birthday ?? "2009-01-01",
      addressLine1: existing?.addressLine1 ?? "1 Test Road",
      addressLine2: existing?.addressLine2 ?? "",
      city: existing?.city ?? "Hobart",
      region: existing?.region ?? "Tasmania",
      postalCode: existing?.postalCode ?? "7000",
      addressCountry: existing?.addressCountry ?? "Australia",
